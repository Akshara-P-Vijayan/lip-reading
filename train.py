...
root_dir = '/kaggle/working/validation'

# Destination directory for the converted frames
dest_dir = '/kaggle/working/validation_frames'
        ...
        for video_folder in os.listdir(label_folder_path):
            ...
            if os.path.isdir(video_folder_path):
                ...
                for video_file in video_files:
                    ...
                    all_val_frames_final.append(frames)

                    # Append label of the video to the labels list
                    all_val_labels_final.append(label_folder)

                    # Print the number of frames extracted
                    print(f"Label: {label_folder}, Video: {video_folder}, Frames extracted: {len(frames)}")

# Convert the list of frames to a numpy array
all_val_frames_final_array = np.array(all_val_frames_final)

# Convert the list of labels to a numpy array
all_val_labels_final_array = np.array(all_val_labels_final)

# Ensure the number of training videos matches the number of training labels
assert len(all_val_frames_final_array) == len(all_val_labels_final_array), "Number of training videos and labels don't match"

print("All training videos have been processed and their labels have been assigned.")
print("Total training videos shape:", all_val_frames_final_array.shape)
print("Total training labels shape:", all_val_labels_final_array.shape)
